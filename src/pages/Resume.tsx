      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-8">
          <section className="glass-panel p-6 animate-slide-up">
            <h2 className="text-2xl font-semibold mb-6 flex items-center text-blue-400">
          ...
          <section className="glass-panel p-6 animate-slide-up" style={{ animationDelay: '0.1s' }}>
            ...
            <ul className="space-y-3">
              {knowledgeList.map((item: string) => <li key={item} className="flex items-center text-gray-300"><Star size={14} className="mr-3 text-blue-500" />{item}</li>)}
            </ul>
          </section>
          ...
          <section className="glass-panel p-6 animate-slide-up" style={{ animationDelay: '0.2s' }}>
            ...
            <ul className="space-y-3">
              {skillsList.map((item: string) => <li key={item} className="flex items-center text-gray-300"><Star size={14} className="mr-3 text-blue-500" />{item}</li>)}
            </ul>
          </section>
          ...
          <section className="glass-panel p-6 animate-slide-up" style={{ animationDelay: '0.3s' }}>
            ...
            <ul className="space-y-3">
              {toolsList.map((item: string) => <li key={item} className="flex items-center text-gray-300"><Star size={14} className="mr-3 text-blue-500" />{item}</li>)}
            </ul>
          </section>
        </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          <section className="glass-panel p-8 animate-slide-up" style={{ animationDelay: '0.4s' }}>
            <h2 className="text-3xl font-semibold mb-8 flex items-center text-blue-400 border-b border-white/10 pb-4">
              <Briefcase size={28} className="mr-4" /> {experience}
            </h2>
            <div className="space-y-8">
              {experienceList.map((exp: any, index: number) => (
                <div key={exp.company} className={`relative pl-8 before:absolute before:left-2 before:top-2 before:w-2 before:h-2 before:bg-blue-500 before:rounded-full before:ring-4 before:ring-blue-500/20 ${index < experienceList.length - 1 ? 'pb-8 border-l border-dashed border-white/20' : ''}`}>
                  <h3 className="text-xl font-bold text-gray-100">{exp.title}</h3>
                  <a href={exp.link} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300 transition-colors">{exp.company}</a>
          ...
          <section className="glass-panel p-8 animate-slide-up" style={{ animationDelay: '0.5s' }}>
            ...
            <div className="space-y-8">
              {educationList.map((edu: any, index: number) => (
                <div key={edu.institution} className={`relative pl-8 before:absolute before:left-2 before:top-2 before:w-2 before:h-2 before:bg-blue-500 before:rounded-full before:ring-4 before:ring-blue-500/20 ${index < educationList.length - 1 ? 'pb-8 border-l border-dashed border-white/20' : ''}`}>
                  <h3 className="text-xl font-bold text-gray-100">{edu.course}</h3>
                  <a href={edu.link} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300 transition-colors">{edu.institution}</a>
