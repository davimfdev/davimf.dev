...
describe('Badge', () => {
  it('usa o tom neutral por padrão', () => {
    ...
    expect(screen.getByText('ATIVA').className).toContain('text-fg-muted');
  });

  it('aplica o tom pedido', () => {
    render(<Badge tone="danger">REVOGADA</Badge>);
    expect(screen.getByText('REVOGADA').className).toContain('text-danger');
  });
  ...
  it('o tom warn tem borda própria, não só cor', () => {
    ...
    expect(screen.getByText('PENDENTE').className).toContain('border-warn');
  });
});
